...
function newSession() {
  ...
  clearMessages()
  addMessage("assistant", "\u65b0\u4f1a\u8bdd\u5df2\u521b\u5efa\uff0c\u8f93\u5165\u6d88\u606f\u5f00\u59cb\u5bf9\u8bdd\u3002")
}
...
async function selectSession(tid) {
  ...
  try {
    ...
    ;(data.messages || []).forEach((m) => {
      if (m.role === "user") {
        addMessage("user", m.content, { image: m.image, file: m.file ? { name: m.file, text: m.file_text, size: m.file_text ? new Blob([m.file_text]).size : null, type: m.file.split(".").pop().toUpperCase(), onClick: () => {} } : null })
...
function onSend(body) {
  ...
  if (body.attachment_text) { fullBody.attachment_text = body.attachment_text; fullBody.attachment_name = body.attachment_name }
  const fileObj = body.attachment_name ? { name: body.attachment_name, text: body.attachment_text, size: body.attachment_text ? new Blob([body.attachment_text]).size : null, type: body.attachment_name.split(".").pop().toUpperCase(), onClick: () => {} } : null
  addMessage("user", body.message, { image: body.image, file: fileObj })
  sendMessage(fullBody)
  loadSessions()
}

function onApprove() { resumeChat("approved") }
function onDeny() { resumeChat("denied") }
function onAbort() { abort() }
function onSynced() { store.syncPending = false; loadSessions() }
function onReverted() { store.syncPending = false }
function onWorkspaceSaved(path) { store.workspace = path; wsModalOpen.value = false }
...
async function onUploadRag(file) {
  ...
  try {
    ...
    typing.value = false
    addMessage("assistant", "\u2705 \u6587\u6863\u5df2\u5165\u5e93: " + data.filename + "\uff08" + data.chunks + " \u4e2a\u7247\u6bb5\uff09\u3002\u4f60\u53ef\u4ee5\u8ba9\u6211\u68c0\u7d22\u6587\u6863\u5185\u5bb9\u3002")
    loadDocuments()
  } catch (err) {
    typing.value = false
...
watch(() => store.currentTid, () => { if (store.authed) loadSessions() })

onMounted(() => { checkAuth() })
</script>

<template>
  <AuthView v-if="!store.authed" @authed="onAuthed" />
  <template v-else>
