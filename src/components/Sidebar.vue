...
function startRename(s) {
  ...
  renameValue.value = s.title || s.thread_id.slice(0, 18) + '…'
}
  ...
  <div class="sidebar">
    ...
    <div class="session-list">
      <div
        ...
        :key="s.thread_id"
        class="session-item"
        :class="{ active: s.thread_id === currentTid }"
      ...
      >
        ...
        >
          <template v-if="renamingId === s.thread_id">
            <input
              ...
              class="rename-input"
              @click.stop
              @keyup.enter="finishRename(true)"
              @keyup.escape="finishRename(false)"
        ...
        <button
          class="del-btn"
          title="删除"
          @click.stop="emit('delete-session', s.thread_id)"
        >
          ✕
        </button>
      </div>
    ...
    <div class="doc-section">
      ...
      <div
        ...
        style="font-size: 12px; color: var(--muted); padding: 4px 8px"
      >
        暂无文档
      </div>
      <div
      ...
      >
        ...
        <button
          class="doc-del"
          title="删除"
          @click="emit('delete-document', d.id)"
        >
          ✕
        </button>
      </div>
